...
class ExternalCFRAgent():
    ...
    def __init__(self, env, model_path='./external_cfr_model'):
        ...
        # self.needs_update = collections.defaultdict(bool)
        # self.calculated = collections.defaultdict(bool)
    ...
    def traverse_tree(self, player_id):
        ...
        if not current_player == player_id:
            action = self.get_action(obs, legal_actions)
            self.env.step(action)
            state_utility = self.traverse_tree(player_id)
            self.update_strategy_sum(obs, action_probs, legal_actions)
    ...
    def update_strategy_sum(self, obs, action_probs, legal_actions):
        for action in range(self.env.num_actions):
            self.average_policy[obs][action] += self.policy[obs][action] * self.iteration
